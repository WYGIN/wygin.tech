import { mysqlTable, mysqlSchema, AnyMySqlColumn, uniqueIndex, index, varchar, text, int, mysqlEnum, datetime, tinyint, primaryKey } from "drizzle-orm/mysql-core"
...
(table) => {
	return {
		uniqueProviderProviderAccountId: uniqueIndex("unique__provider__provider_account_id").on(table.provider, table.providerAccountId),
...
export const blogs = mysqlTable("blogs", {
	...
	status: mysqlEnum("status", ['published','draft','scheduled','archived']).default('published').notNull(),
...
(table) => {
	return {
		uniquePostIdBlogIdUserId: uniqueIndex("unique__post_id__blog_id__user_id").on(table.postId, table.blogId, table.userId),
		indexUserId: index("index__user_id").on(table.userId),
	}
});
...
(table) => {
	return {
		uniqueCategory: uniqueIndex("unique__category").on(table.category1, table.category2),
		indexCategory1: index("index_category1").on(table.category1),
		indexCategory2: index("index_category2").on(table.category2),
	}
});

export const categoryFollowers = mysqlTable("category_followers", {
	categoryId: varchar("category_id", { length: 12 }),
	userId: varchar("user_id", { length: 12 }),
},
(table) => {
	return {
		uniquePublicationIdUserId: uniqueIndex("unique__publication_id__user_id").on(table.categoryId, table.userId),
...
export const footers = mysqlTable("footers", {
	id: varchar("id", { length: 12 }).primaryKey().notNull(),
	logo: text("logo").default(sql`('https://wygin.tech/wygin.svg')`).notNull(),
...
(table) => {
	return {
		uniqueSlugCategoryId: uniqueIndex("unique__slug__category_id").on(table.slug, table.categoryId),
		uniqueTitle: uniqueIndex("unique_title").on(table.title),
		indexId: index("index__id").on(table.id),
		indexTitle: index("index__title").on(table.title),
		indexSlugCategoryId: index("index__slug__category_id").on(table.slug, table.categoryId),
	}
});

export const publicationFollowers = mysqlTable("publication_followers", {
	publicationId: varchar("publication_id", { length: 12 }).notNull(),
	userId: varchar("user_id", { length: 12 }).notNull(),
},
(table) => {
	return {
		uniquePublicationIdUserId: uniqueIndex("unique__publication_id__user_id").on(table.publicationId, table.userId),
		indexPublicationId: index("index__publication_id").on(table.publicationId),
		indexUserId: index("index__user_id").on(table.userId),
		publicationFollowersPublicationIdUserId: primaryKey(table.publicationId, table.userId)
	}
});

export const publicationRoles = mysqlTable("publication_roles", {
	id: varchar("id", { length: 12 }).primaryKey().notNull(),
	publicationId: varchar("publication_id", { length: 12 }).notNull(),
	userId: varchar("user_id", { length: 12 }).notNull(),
	role: mysqlEnum("role", ['user','editor','contributor','sponser','admin','owner']).default('user').notNull(),
},
(table) => {
	return {
		uniquePublicationIdUserId: uniqueIndex("unique__publication_id__user_id").on(table.publicationId, table.userId),
...
(table) => {
	return {
		uniqueSessionToken: uniqueIndex("unique__session_token").on(table.sessionToken),
		indexId: index("index__id").on(table.id),
		indexUserId: index("index__user_id").on(table.userId),
	}
});

export const tagFollowers = mysqlTable("tag_followers", {
	tagId: varchar("tag_id", { length: 12 }),
	userId: varchar("user_id", { length: 12 }),
},
(table) => {
	return {
		uniquePublicationIdUserId: uniqueIndex("unique__publication_id__user_id").on(table.tagId, table.userId),
...
(table) => {
	return {
		uniqueFollowerIdFollowingId: uniqueIndex("unique__follower_id__following_id").on(table.followerId, table.followingId),
		...
		userFollowsFollowerIdFollowingId: primaryKey(table.followerId, table.followingId)
	}
});
